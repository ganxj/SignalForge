...
from config.config_loader import get_config, PROMPT_FILTER, PROMPT_PREFILTER
...
def build_prefilter_prompt(post: dict) -> List[Dict]:
    """Build a very short gate prompt before the full scoring prompt."""
    return [
        {
            "role": "system",
            "content": "You are a strict product-opportunity gatekeeper. Reject low-signal Reddit content quickly."
        },
        {
            "role": "user",
            "content": f"{config['prompts'][PROMPT_PREFILTER]}\n\n{_format_post_content(post)}"
        }
    ]
